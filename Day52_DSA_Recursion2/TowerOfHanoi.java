...
public class TowerOfHanoi {
    ...
    public void toh(int n, int src, int dest, int temp, ArrayList<ArrayList<Integer>> ans) {
        ArrayList<Integer> rec = new ArrayList<>();
        if (n == 0) {
            return;
        }
        toh(n - 1, src, temp, dest, ans);
        rec.add(n);
        rec.add(src);
        rec.add(dest);
        ans.add(rec);
        toh(n - 1, temp, dest, src, ans);
